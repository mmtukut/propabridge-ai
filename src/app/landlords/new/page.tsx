...
const steps = [
  ...
  { id: 'pricing', title: 'Pricing & Terms', icon: DollarSign },
...
const formSchema = detailsSchema.merge(locationSchema).merge(pricingSchema).merge(mediaSchema);
    ...
    return (
        <div className="space-y-8 text-muted-foreground">
            <div className="space-y-2">
                <h3 className="font-headline text-lg font-semibold text-foreground">Property Details</h3>
             ...
             <div className="space-y-2">
                <h3 className="font-headline text-lg font-semibold text-foreground">Location</h3>
                <p><strong>Address:</strong> {values.address}</p>
                <p><strong>City:</strong> {values.city}</p>
                <p><strong>State:</strong> {values.state}</p>
            </div>
             <div className="space-y-2">
                <h3 className="font-headline text-lg font-semibold text-foreground">Pricing</h3>
             ...
             <div className="space-y-2">
                <h3 className="font-headline text-lg font-semibold text-foreground">Media</h3>
                <p>{values.photos?.length || 0} photos uploaded.</p>
            </div>
        </div>
    );
};
...
export default function NewListingPage() {
  ...
  const schemas = [detailsSchema, locationSchema, pricingSchema, mediaSchema, z.object({})];
            ...
            <FormProvider {...methods}>
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="mt-10 flex justify-between">
                        <Button type="button" variant="secondary" onClick={prevStep} disabled={currentStep === 0} className="px-8">
                            <ArrowLeft className="mr-2" /> Previous
                        </Button>

                        {currentStep < steps.length - 1 ? (
                            <Button type="button" onClick={nextStep} className="px-8 shadow-glow-primary">
                                Next <ArrowRight className="ml-2" />
                            </Button>
                        ) : (
                            <Button type="submit" className="px-8 shadow-glow-primary">
                                Submit Listing <Send className="ml-2" />
                            </Button>
                        )}
                    </div>
                </form>
