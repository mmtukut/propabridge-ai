...
const TrustBadge = ({ value, label }: { value: string; label: string }) => (
  <div className="flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 shadow-lg text-center">
  ...
  return (
    <section className="relative bg-background py-20 md:py-32 overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-40">
          <div className="absolute top-0 left-0 w-96 h-96 bg-primary rounded-full blur-[150px] animate-pulse-slow" />
          <div className="absolute bottom-0 right-0 w-96 h-96 bg-accent rounded-full blur-[150px] animate-pulse-slow animation-delay-2000" />
        </div>
        <div className="container max-w-7xl text-center relative z-10">
            <h1 
              className="text-4xl md:text-6xl font-bold text-foreground mb-6 leading-tight"
              style={{ textShadow: '0 0 10px hsl(var(--primary) / 0.3), 0 0 20px hsl(var(--primary) / 0.2)' }}
            >
